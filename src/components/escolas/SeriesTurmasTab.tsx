...
export function SeriesTurmasTab({ escolaId }: { escolaId: string }) {
  ...
  // Filter out series already created for this school
  const availableSeriesOptions = SERIES_OPTIONS.filter(
    opt => !series.some(s => s.nome === opt)
  );
  ...
  const saveTurma = async (serieId: string) => {
    ...
    if (existing.some(t => t.nome === turmaName)) {
      toast.error(`Turma "${turmaName}" já existe nesta série.`);
      return;
    }

    const salaNum = turmaForm.sala ? parseInt(turmaForm.sala) : null;
    if (turmaForm.sala && (isNaN(salaNum!) || salaNum! <= 0)) {
      toast.error('Número da sala deve ser um valor numérico positivo.');
      return;
    }
    ...
    const { error } = await supabase.from('turmas').insert({
      ...
      serie_id: serieId,
      escola_id: escolaId,
      sala: salaNum ? String(salaNum) : null,
    ...
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" onClick={() => setShowSerieForm(!showSerieForm)} className="gap-1"
          disabled={availableSeriesOptions.length === 0}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={e => setTurmaForm({ ...turmaForm, sala: e.target.value.replace(/\D/g, '') })}
