...
interface MockAluno { id: string; nome: string; matricula: string; iniciais: string; status: StatusType; horaEntrada: string | null; }
...
const avatarColors = ['bg-primary/15 text-primary', 'bg-emerald-100 text-emerald-700', 'bg-amber-100 text-amber-700', 'bg-violet-100 text-violet-700', 'bg-rose-100 text-rose-700', 'bg-cyan-100 text-cyan-700'];
...
export default function TurmaDetalhe() {
  ...
  useEffect(() => {
    ...
    const { data: t } = db.turmas.getById(turmaId);
    const { data: e } = db.escolas.getById(escolaId);
    setTurma(t as any);
    setEscola(e as any);
    setLoading(false);
  }, [turmaId, escolaId]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Card><CardContent className="p-5 flex items-center gap-4"><div className="h-11 w-11 rounded-xl bg-violet-500/10 flex items-center justify-center"><GraduationCap className="h-5 w-5 text-violet-600" /></div><div><p className="text-sm text-muted-foreground">Professores</p><p className="text-base font-semibold text-card-foreground">Profª. Claudia Reis</p></div></CardContent></Card>
        <Card><CardContent className="p-5 flex items-center gap-4"><div className="h-11 w-11 rounded-xl bg-primary/10 flex items-center justify-center"><Users className="h-5 w-5 text-primary" /></div><div><p className="text-sm text-muted-foreground">Total de Alunos</p><p className="text-2xl font-bold tabular-nums text-card-foreground">{MOCK_ALUNOS.length}</p></div></CardContent></Card>
        <Card><CardContent className="p-5 flex items-center gap-4"><div className="h-11 w-11 rounded-xl bg-emerald-500/10 flex items-center justify-center"><TrendingUp className="h-5 w-5 text-emerald-600" /></div><div><p className="text-sm text-muted-foreground">Frequência do Dia</p><p className="text-2xl font-bold tabular-nums text-card-foreground">{Math.round(((counts.presentes + counts.atrasados) / MOCK_ALUNOS.length) * 100)}%</p></div></CardContent></Card>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-9 w-9 rounded-lg bg-emerald-500/10 flex items-center justify-center"><span className="text-emerald-600 font-bold text-sm">{counts.presentes}</span></div><div><p className="text-xs text-muted-foreground">Presentes</p><p className="text-lg font-bold tabular-nums">{Math.round((counts.presentes / MOCK_ALUNOS.length) * 100)}%</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-9 w-9 rounded-lg bg-amber-500/10 flex items-center justify-center"><span className="text-amber-600 font-bold text-sm">{counts.atrasados}</span></div><div><p className="text-xs text-muted-foreground">Atrasados</p><p className="text-lg font-bold tabular-nums">{counts.atrasados}</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-9 w-9 rounded-lg bg-destructive/10 flex items-center justify-center"><span className="text-destructive font-bold text-sm">{counts.faltas}</span></div><div><p className="text-xs text-muted-foreground">Faltas</p><p className="text-lg font-bold tabular-nums">{counts.faltas}</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-9 w-9 rounded-lg bg-blue-500/10 flex items-center justify-center"><span className="text-blue-600 font-bold text-sm">{counts.justificadas}</span></div><div><p className="text-xs text-muted-foreground">Justificadas</p><p className="text-lg font-bold tabular-nums">{counts.justificadas}</p></div></CardContent></Card>
      </div>
                ...
                return (
                  <TableRow key={aluno.id}>
                    ...
                    <TableCell className="text-right">
                      {aluno.horaEntrada ? <span className="inline-flex items-center gap-1 text-sm tabular-nums"><Clock className="h-3.5 w-3.5 text-muted-foreground" />{aluno.horaEntrada}</span> : <span className="text-sm text-muted-foreground">—</span>}
                    </TableCell>
                  </TableRow>
                );
              })}
