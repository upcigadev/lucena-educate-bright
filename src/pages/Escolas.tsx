    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((escola, idx) => (
            <Card key={escola.id} className={cn('border-l-4 hover:shadow-md transition-all cursor-pointer group', cardColors[idx % cardColors.length])} onClick={() => navigate(`/escolas/${escola.id}`)}>
              <CardContent className="p-5">
                ...
                <div className="space-y-1.5 mb-4">
                  {escola.endereco && <p className="text-xs text-muted-foreground flex items-center gap-1.5"><MapPin className="h-3 w-3 shrink-0" /> {escola.endereco}</p>}
                  {escola.telefone && <p className="text-xs text-muted-foreground flex items-center gap-1.5"><Phone className="h-3 w-3 shrink-0" /> {escola.telefone}</p>}
                </div>
                <Button variant="ghost" size="sm" className="w-full justify-between text-primary group-hover:bg-primary/5">
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          <SheetHeader><SheetTitle>Nova Escola</SheetTitle></SheetHeader>
          <div className="space-y-4 mt-4">
            <div className="space-y-2"><Label>Nome *</Label><Input value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} /></div>
            <div className="space-y-2"><Label>INEP</Label><Input value={form.inep} onChange={e => setForm({ ...form, inep: e.target.value })} /></div>
